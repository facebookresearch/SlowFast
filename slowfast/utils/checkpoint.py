...
def save_checkpoint(path_to_job, model, optimizer, epoch, cfg):
    ...
    normalized_sd = split_to_normal_bn(sd)
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        for key in caffe2_checkpoint["blobs"].keys():
            ...
            converted_key = c2_normal_to_split_bn(
        ...
        checkpoint["model_state"] = normal_to_split_bn(
...
def split_to_normal_bn(sd):
    """
    Convert the split-BN paprameters to normal BN parameters in a state dict.
    There are two copies of BN layers in a split-BN implementation: `bn.bn` and
    `bn.split_bn`. `bn.split_bn` is used during training and
    "compute_precise_bn". Before saving or evaluation, its stats are copied to
    `bn.bn`. We rename `bn.bn` to `bn` and store it to be consistent with normal
    BN layers.
    Args:
        sd (OrderedDict): a dict of parameters whitch might contain split-BN
        parameters.
    Returns:
        new_sd (OrderedDict): a dict with split-BN parameters reshaped to
        normal parameters.
    """
...
def c2_normal_to_split_bn(key, model_keys):
    """
    Convert BN parameters to Split-BN parameters if model contains Split-BNs.
    Args:
...
def normal_to_split_bn(checkpoint_sd, model_sd):
    """
    Convert BN parameters to Split-BN parameters if model contains Split-BNs.
    Args:
