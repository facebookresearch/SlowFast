...
import numpy as np
import slowfast.utils.checkpoint_deprecated as cu
...
def test(cfg):
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH is not "":
        cu.load_checkpoint(
            cfg.TEST.CHECKPOINT_FILE_PATH, model, cfg.NUM_GPUS > 1
        )
    elif cu.has_checkpoint(cfg.OUTPUT_DIR):
        last_checkpoint = cu.get_last_checkpoint(cfg.OUTPUT_DIR)
        cu.load_checkpoint(last_checkpoint, model, cfg.NUM_GPUS > 1)
    elif cfg.TRAIN.CHECKPOINT_FILE_PATH is not "":
        # If no checkpoint found in TEST.CHECKPOINT_FILE_PATH or in the current
        # checkpoint folder, try to load checkpint from
        # TRAIN.CHECKPOINT_FILE_PATH and test it.
        cu.load_checkpoint(
            cfg.TRAIN.CHECKPOINT_FILE_PATH, model, cfg.NUM_GPUS > 1
        )
    else:
